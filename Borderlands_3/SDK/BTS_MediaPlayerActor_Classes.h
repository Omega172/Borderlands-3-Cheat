...
namespace CG
{
	...
	public:
		...
		unsigned char                                              UnknownData_YD9Z[0x3];                                   // 0x0545(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_HYS4[0x3];                                   // 0x0555(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		class UTimelineComponent*                                  timerPlaySlide;                                          // 0x0558(0x0008) BlueprintVisible, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash
		...
		unsigned char                                              UnknownData_IWVK[0x7];                                   // 0x0561(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_B30H[0x3];                                   // 0x0575(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_MMEZ[0x4];                                   // 0x05BC(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_4XWK[0x3];                                   // 0x05D9(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_ZR07[0x4];                                   // 0x05F4(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_W31V[0x3];                                   // 0x0605(0x0003) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
		...
		unsigned char                                              UnknownData_AOIQ[0x4];                                   // 0x062C(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	public:
		void slideShowsetShowSlideTextures(bool Visible);
		void slideShowSetLoadingSpinnerVisibility(bool IsVisible);
		int32_t getWrappedIndex(int32_t Index, int32_t arrayLength);
		void slideShowHandleLoadedSlide(class UObject* Object);
		void getItemWrapped(int32_t Index);
		bool IsAssetLoaded();
		void videoseekComplete();
		void slideShowIncrementActiveSlide(int32_t* activeSlideIndex);
		void SetProgressBarProgress(float Progress, int32_t CurrentFrame, int32_t NumberOfFrames);
		void slideShowSetSlideA(int32_t Index);
		void slideShowSetSlideB(int32_t Index);
		void videoSeekForward();
		void videoSeekBack();
		void videoapplyPendingSeek();
		void slideShowGetTextureMaterialParams(class UTexture2D* Texture2D, struct FLinearColor* LinearColor);
		void UserConstructionScript();
		void timerPlaySlide__FinishedFunc();
		void timerPlaySlide__UpdateFunc();
		...
		void slideShowPausePressed();
		void slideShowSkipBackPressed();
		void slideShowSkipForwardPressed();
		void slideShowPlayTransition();
		void slideShowPlayPressed();
		void showplaybackWindow();
		void slideShowplaySlide();
		void loadMedia();
		void ReceiveBeginPlay();
		void LoadAdjacentTextures();
		void loadNextTexture();
		void ceslideshowTransitionToNextSlide();
		void ceslideshowtransitionToPreviousSlide();
		void EndOfVideoReached();
		void slideShowplaySlideFromStart();
		void TracksChanged();
		void mediaRestart();
		void MediaClosed();
