#pragma once

/**
 * Name: Borderlands_3
 * Version: OAK-PATCHWIN640-328
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function BPCont_TedioreTurret_SpiderMind.BPCont_TedioreTurret_SpiderMind_C.UserConstructionScript
	 */
	struct ABPCont_TedioreTurret_SpiderMind_C_UserConstructionScript_Params
	{	};

	/**
	 * Function BPCont_TedioreTurret_SpiderMind.BPCont_TedioreTurret_SpiderMind_C.BndEvt__Targeting_K2Node_ComponentBoundEvent_4_TargetingComponentTargetListChangedSignature__DelegateSignature_BPCont_TedioreTurret
	 */
	struct ABPCont_TedioreTurret_SpiderMind_C_BndEvt__Targeting_K2Node_ComponentBoundEvent_4_TargetingComponentTargetListChangedSignature__DelegateSignature_BPCont_TedioreTurret_Params
	{
	public:
		class UTargetingComponent*                                 TargetingComponent;                                      // 0x0000(0x0008)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function BPCont_TedioreTurret_SpiderMind.BPCont_TedioreTurret_SpiderMind_C.ExecuteUbergraph_BPCont_TedioreTurret_SpiderMind
	 */
	struct ABPCont_TedioreTurret_SpiderMind_C_ExecuteUbergraph_BPCont_TedioreTurret_SpiderMind_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
		unsigned char                                              UnknownData_6SDF[0x4];                                   // 0x0004(0x0004) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY) ()
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
